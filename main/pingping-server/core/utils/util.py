...
def check_model_key(modelType, modelKey):
    if "你" in modelKey:
        return f"Configuration error: {modelType} API key is not set, current value: {modelKey}"
    return None
...
def check_ffmpeg_installed() -> bool:
    ...
    Returns:
        bool: Returns True if ffmpeg is available; otherwise raises ValueError exception.

    Raises:
        ValueError: When ffmpeg is not installed or dependencies are missing, raises detailed error message.
    """
    try:
        ...
        # If version information is not detected, treat as abnormal
        ...
        error_msg = [
            "❌ Detected that ffmpeg cannot run normally.\n",
            "Suggestions:",
            "1. Confirm that the conda environment is correctly activated;",
            "2. Check the project installation documentation to learn how to install ffmpeg in the conda environment.\n",
        ]

        # 🎯 Provide additional hints for specific error messages
        if "libiconv.so.2" in stderr_output:
            error_msg.append("⚠️ Missing dependency library detected: libiconv.so.2")
            error_msg.append("Solution: Execute in the current conda environment:")
            ...
            error_msg.append("⚠️ System could not find ffmpeg executable.")
            error_msg.append("Solution: Execute in the current conda environment:")
        ...
        # Raise detailed exception information
