...
class ConnectionHandler:
    def __init__(
        ...
        self.bind_prompt_interval = 30  # Bind prompt playback interval (seconds)
        ...
        self.first_activity_time = 0.0  # Record time of first activity (milliseconds)
        self.last_activity_time = 0.0  # Unified activity timestamp (milliseconds)
        self.client_voice_stop = False
        self.last_is_voice = False

        # ASR related variables
        # Because shared local ASR may be used in actual deployment, variables cannot be exposed to shared ASR
        ...
        # Handle TTS response without text return
        ...
        )  # Add 60 seconds on top of the first timeout, for second-level timeout
        self.timeout_task = None

        # {"mcp":true} indicates MCP feature is enabled
        self.features = None
    ...
    async def handle_connection(self, ws):
        try:
            ...
            self.welcome_msg = self.config["pingping"]
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            ...
            # When header processing is not needed or no header exists, process raw message directly
    ...
    async def _process_mqtt_audio_message(self, message):
        ...
        try:
            ...
            if audio_length > 0 and len(message) >= 16 + audio_length:
                # If length is specified, extract precise audio data
                audio_data = message[16 : 16 + audio_length]
                # Process based on timestamp sorting
                self._process_websocket_audio(audio_data, timestamp)
                return True
            elif len(message) > 16:
                # If no length specified or length invalid, remove header and process remaining data
